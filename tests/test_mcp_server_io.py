...
def test_extract_pair_id_from_mcp_server_json():
    pair_id = extract_pair_id_from_mcp_server_json(
        json.dumps(
            {
                "pair_id": "00000000-0000-4000-8000-000000000001",
            }
...
def test_legacy_redact_policy_value_now_refuses_with_deprecation_warning(
    ...
    deprecation_records = [
        r for r in caplog.records if "DEPRECATED" in r.message and "redact" in r.message
    ]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "ya29.A0ARrdaMabcdefghijklmnopqrstuvwxyz0123456789",  # Google OAuth
        "glpat-abcdefghij1234567890",  # GitLab PAT
        "xoxp-1234567890-abcdefghijklmnop",  # Slack user token (broadened from xoxb)
        "sk_live_abcdefghij1234567890",  # Stripe secret key
    ],
)
def test_value_shape_detects_broadened_credential_menu_outside_secret_fields(
    literal: str,
) -> None:
    """Amendment 005/Group 2 (finding D): a high-confidence credential shape is
    flagged even under a non-secret field name and outside any env/headers block,
    via the value-shape leg of detection."""
    findings = find_mcp_secret_literals(
        {
            "mcpServers": {"svc": {"settings": {"opaque_value": literal}}},
        }
    )
    assert [f.field_path for f in findings] == [
        "mcpServers.svc.settings.opaque_value",
    ]
...
def test_secrets_accepted_logs_warning_with_new_message(
    ...
    assert any("secret_policy=secrets_accepted" in r.message for r in caplog.records)
...
def test_secret_finding_field_path_distinguishes_list_index_from_dict_key():
    ...
    list_indexed = find_mcp_secret_literals({"servers": [{"token": "ghp_abcdefghijkl"}]})
    assert len(list_indexed) == 1
    assert list_indexed[0].field_path == "servers[0].token"

    dict_key_zero = find_mcp_secret_literals({"servers": {"0": {"token": "ghp_abcdefghijkl"}}})
