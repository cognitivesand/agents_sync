...
def _set_canonical_lm(state_dir: Path, pair_id: str, last_modified: float, generation: int = 0) -> None:
...
def test_export_attaches_last_modified_from_canonical_metadata(syncer: Syncer, tmp_path: Path):
    ...
    _set_canonical_lm(syncer.state_dir, pair_id, last_modified=ps.last_modified, generation=ps.generation)
    ...
    assert doc["metadata"]["last_modified"] == ps.last_modified
    assert doc["metadata"]["generation"] == ps.generation


def test_import_last_modified_wins_ignores_generation_uses_wall_clock(syncer: Syncer, tmp_path: Path):
    ...
    from agents_sync.canonical import canonical_last_modified
...
def test_import_pair_id_collision_last_modified_wins_tie_keeps_local(syncer: Syncer, tmp_path: Path):
...
def test_import_partial_promotion_adopted_by_next_sync(
    syncer: Syncer, tmp_path: Path, monkeypatch
):
