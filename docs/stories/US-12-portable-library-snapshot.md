...
- [ ] AC-3 [Normal]: Given a managed customization_artifact, When it is written into the export, Then the in-archive copy is augmented with a per-artifact `last_modified` field whose value is the maximum `last_seen` timestamp across the artifact's per-agentic_tool state entries. The on-disk canonical is not modified.
...
- [ ] AC-5 [Normal]: Given a zip produced by AC-1 and a fresh install (no managed customization_artifacts locally), When the user runs `agents-sync import <file.zip>`, Then every canonical document in the zip is written to the local state directory, each customization_artifact gets a `state.json` entry with empty `last_written` for every locally-enabled, supported agentic_tool, and the next `sync_once` projects every imported artifact to every `available` agentic_tool. No archive entries are created (no existing user-authored bytes were displaced).
- [ ] AC-6 [Normal]: Given the import zip carries a customization_artifact with the same `customization_artifact_id` as a locally-managed one, When `import` runs under the configured `import_collision_strategy`, Then the strategy is applied:
  ...
  - `mtime_wins` (default): the candidate with the higher `last_modified` value replaces the loser. If the import wins, the local canonical is overwritten and every agentic_tool's tool-side file is archived (NFR-01) before re-projection on the next poll. If the local wins, the imported artifact is ignored.
