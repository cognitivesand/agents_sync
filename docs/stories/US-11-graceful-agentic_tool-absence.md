...
- [ ] AC-9 [Normal — an emptied tool is treated as removed]: Given an enabled, `available` agentic_tool whose root is present and readable but which, at a poll, holds **none** of the customization_artifacts that `state` records for it (every recorded artifact has vanished — the signature of an uninstall that left an empty root), When the watcher polls, Then the agentic_tool is reclassified `unavailable` for that poll, so by AC-2 its state entries are preserved verbatim, it is **not** a removal source (the vanished artifacts are not propagated as deletions) and **not** a projection target (AC-8 heal does not fire on it) — the synced library is preserved with no user interaction (NFR-17). A structured INFO line names the agentic_tool and the bulk disappearance. The tool returns to `available` and re-extends per AC-3 when at least one of its artifacts reappears (e.g. the tool is reinstalled). A **partial** disappearance — at least one recorded artifact still present on the tool — leaves the tool `available`, and each vanished artifact remains a normal removal signal per AC-4 / US-05 AC-2.
