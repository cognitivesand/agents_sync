...
- [ ] AC-9 [Failure — bulk disappearance on an available agentic_tool]: Given an enabled, `available` agentic_tool whose root is present and readable but from which **every** customization_artifact that `state` recorded as held on it has disappeared within a single poll (the signature of an uninstall that left an empty root, or a present-but-empty mount point), When the watcher polls, Then the daemon **shall not** propagate those removals and **shall not** drop any `pair_id`: it logs a structured WARNING naming the agentic_tool and the count of vanished artifacts, and preserves the tool's state entries verbatim — treating the bulk disappearance as a tool-availability event, not a user deletion, and (as an explicit exception to AC-8) it does **not** re-project them either. A **partial** disappearance — at least one recorded artifact still present on the tool — is unaffected and remains a normal removal signal per AC-4 / US-05 AC-2.
