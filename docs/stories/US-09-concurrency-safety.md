...
- [ ] AC-4 [Failure]: Given the canonical for a customization_artifact is detected as truncated or unparseable on read, When the tool reads it, Then the canonical is treated as missing for this customization_artifact, a structured error is logged, the truncated canonical is moved to quarantine (its bytes preserved for recovery, separate from the user-content archive), and on the next poll the canonical is rebuilt from the agentic_tool with the most recent `mtime`.
