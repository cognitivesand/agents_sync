...
- [ ] ~~AC-4~~ *(retired — amendment 019)*: The user-facing `private` flag is removed. Artifacts are synced regardless of any `private:` field; no propagation/adoption gate is keyed on it. (Framework-specific hold-back is unaffected — see US-15.)
...
- The `provenance: "user" | "agent"` field is informational only in v0.5 — it is recorded on the canonical for diagnostics and future reference but has no associated policy. Sources of `provenance: "agent"`: Goose memory extension, Claude Code memory tool output, Gemini CLI `/memory add`-appended blocks. Adapters set it at parse time based on the source path.
