...
- **FR-07** (Customization library export/import): The daemon **shall** provide one-shot CLI subcommands (`export`, `import`) that serialise the managed customization library to a single transportable file and deserialise such a file back into a local install. The customization library export **shall** contain only host-portable content (canonical documents and a manifest); `state.json` and the on-disk archive directory **shall not** be included.
- **FR-08** (Configurable import collision strategy): When importing a customization library, the daemon **shall** support a configurable collision strategy (`skip`, `mtime_wins`, `overwrite`) governing how an imported customization_artifact is reconciled with a locally-managed one of the same `customization_artifact_id` or of the same `(customization_type, target_slug(name))`. The default **shall** be `mtime_wins`. The strategy **shall** be overridable per invocation on the CLI.
- **FR-09** (rules matrix): The daemon **shall** sync user-level `rules` customization_artifacts across every available agentic_tool whose `supported_customization_types` includes `rules`.
- **FR-10** (slash_command matrix): The daemon **shall** sync user-level `slash_command` customization_artifacts across every available agentic_tool whose `supported_customization_types` includes `slash_command`.
- **FR-11** (mcp_server matrix): The daemon **shall** sync user-level `mcp_server` customization_artifacts across every available agentic_tool whose `supported_customization_types` includes `mcp_server`.
- **FR-12** (Customization library export — secret-policy application): When the user invokes the customization library export subcommand, the daemon **shall** apply `mcp_server_secret_policy` to every `mcp_server` artifact included in the export.
- **FR-13** (Customization library export — `secrets_accepted` interlock): If a customization library export would contain literal secret material under `mcp_server_secret_policy = secrets_accepted`, then the daemon **shall** abort the export unless the operator has explicitly opted in to including secret material on the same invocation.
- **FR-14** (Customization library export — secret-content declaration): The customization library export **shall** record in its manifest (a) whether the export contains literal secret material and (b) the list of affected `customization_artifact_id`s.
- **FR-15** (Customization library import — secret-policy application): When the user invokes the customization library import subcommand, the daemon **shall** apply the active `mcp_server_secret_policy` to every imported `mcp_server` artifact, irrespective of the source-host policy that produced the export.
...
- **NFR-15** (Secret handling): The daemon **shall** honour the configured `mcp_server_secret_policy` for every `mcp_server` artifact. Accepted values are `secrets_refused` (default), `secrets_redacted`, and `secrets_accepted`. Under `secrets_refused`, the daemon **shall** fail closed for any `mcp_server` artifact whose canonical document contains a literal value matching the secret-detection heuristics on any of `env`, `headers`, or `auth.*`. Under `secrets_redacted`, literals **shall** be replaced with `${env:AGENTS_SYNC_REDACTED_<n>}` placeholders and the redaction **shall** be recorded in the artifact's canonical document. Under `secrets_accepted`, literals **shall** propagate verbatim and the daemon **shall** log one structured warning per affected artifact per poll. The `mcp_server_secret_policy` **shall** be applied at every artifact-egress boundary, not only at parse time — this includes customization library export and customization library import.
