...
- **NFR-15** (Secret handling): The daemon **shall** apply the configured `secret_policy` at every artifact-egress boundary. Accepted values are `secrets_refused` (default) and `secrets_accepted`.
