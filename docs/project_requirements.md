...
- **NFR-11** (Extensibility): Adding support for a new agentic_tool **shall** require only a new agentic_tool integration module plus a matching configuration entry; adding a new customization_type **shall** require only declaring that type and its on-disk file layout. No change to the shared sync mechanism — discovery, adoption, reconciliation, conflict resolution, or removal propagation — **shall** be required. The concrete integration contract (module layout, function signatures, registration) is specified in `docs/agentic_tool_integration_protocol.md`.
...
- **NFR-18** (Thin adapter over shared mechanism): An agentic_tool integration module **shall** perform its bridging role as defined in `docs/agentic_tool_integration_protocol.md` with the minimum code necessary, and reusing as much as possible code from the shared engine.
