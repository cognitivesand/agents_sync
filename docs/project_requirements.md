...
- **FR-12** (Export-time secret-policy application): When the user invokes the `export` subcommand, the daemon **shall** apply `mcp_server_secret_policy` to every `mcp_server` artifact included in the snapshot.
- **FR-13** (Permissive export interlock): If a portable library snapshot would contain literal secret material under `mcp_server_secret_policy = permissive`, then the daemon **shall** abort the export unless the operator has explicitly opted in to including secret material on the same invocation.
- **FR-14** (Snapshot secret-content declaration): The portable library snapshot manifest **shall** record (a) whether the snapshot contains literal secret material and (b) the list of affected `customization_artifact_id`s.
- **FR-15** (Import-time secret-policy application): When the user invokes the `import` subcommand, the daemon **shall** apply the active `mcp_server_secret_policy` to every imported `mcp_server` artifact, irrespective of the source-host policy that produced the snapshot.
