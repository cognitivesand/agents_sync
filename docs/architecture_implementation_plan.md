...
| S8b | compute_sync_plan — key-conflict guards | `plan/compute_sync_plan` (+ grows `sync_plan` intents) | the whole-poll **managed-key index** drives two downgrades that both ask "does this `(kind, slug)` match a managed artifact?": two already-managed artifacts (different ids) resolving to the same key, or a `rename_artifact` whose new slug clashes with another managed artifact → `reject_collision` (untouched, structured error — US-04 AC-5, US-03 AC-8); a candidate group whose key matches an already-managed artifact → `absorb_into_managed` instead of `adopt_new_artifact` (managed wins, no mint — US-03 AC-6). Grows `sync_plan` with the `RejectCollision` + `AbsorbIntoManaged` payloads. Pure, no FS |
| S8c | compute_sync_plan — glitch guard | `plan/compute_sync_plan` | the per-tool whole-poll vanish count: a `remove_artifact` on a tool that suffered a *glitch* — ≥2 of its recorded artifacts vanished this poll → `reproject_canonical` instead (restore from canonical, no removal propagated — US-11 AC-9); exactly one vanished recorded artifact stays a deliberate deletion. Redirects existing intents (no new payload). Pure, no FS. (Framework-specific projection hold-back is US-15, at S12 with its read-phase flag; the `private` flag was retired — amendment 019.) |
