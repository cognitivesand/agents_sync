...
- **Phase F:** S20 increment 1 ✓ (0.7.33 — tools-as-data core); increment 2 ✓ (0.7.34 —
  ...
  `name_render_field`; closes the increment-5 codex `per_tool_only` transport drift).
- **Next step → S20 increment 7** — the per-tool inline env-reference *style*
  (`${env:NAME}`↔`${NAME}`↔`{env:NAME}`), cross-cutting (increment 5 fixed the canonical
  `${env:NAME}` representation; 7 converts it to each tool's native inline spelling). **Tracked
  gap:** gemini's `oauth` auth-field spelling is an increment-4-style auth knob gemini still
  lacks (it renders auth under `auth`, not `oauth`) — fold in with increment 7 or a cleanup.
