...
| S6b | Reconcile known — surface-shape guards | `plan/reconcile_known` | one observed-vs-recorded surface/name diff: name moved → `rename_artifact` (collides → `reject_collision`); recorded surface vanished → `remove_artifact`, unless ≥2 vanished on a live tool (glitch) → `reproject_canonical`; pure mv (same digest, new location) → record (US-04/11, US-03 AC-8) |
| S6c | Reconcile known — canonical authority | `plan/reconcile_known` | canonical changed out of band → `reproject_canonical`; canonical-store entry corrupt → `rebuild_corrupt_canonical` (US-09) |
