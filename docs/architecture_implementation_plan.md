...
| S8 | compute_sync_plan + cross-artifact guards | `plan/compute_sync_plan` | whole-plan assembly + the guards that need the whole-poll view and *downgrade* per-artifact intents: two-tool guard (US-07 AC-5); slug clash / two-managed-same-key → `reject_collision` (US-04 AC-5, US-03 AC-8); a candidate group whose key matches a managed artifact → `absorb_into_managed` (US-03 AC-6); glitch (≥2 of a tool's artifacts vanished) → `reproject_canonical` (US-11 AC-9); private/framework predicates (US-13/15) — all pure, no FS |
