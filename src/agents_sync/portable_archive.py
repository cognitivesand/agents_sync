...
def export_to_zip(
    ...
    *,
    ...
    for pair_id, ps in state.items():
        ...
        # Phase 2.2: read last_modified/generation from the nested metadata
        # block (canonical_last_modified). Fall back to PairState for canonicals
        # that predate Phase 1.2 stamping and have no metadata block yet.
        lm = canonical_last_modified(canonical)
        if lm is None:
            lm = ps.last_modified if ps.last_modified is not None else 0.0
        meta = canonical_metadata(canonical)
        gen = int(meta.get("generation", ps.generation))
        set_canonical_metadata(entry, last_modified=lm, generation=gen)
